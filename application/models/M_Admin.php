<?php
    ...
    class M_Admin extends CI_Model{
        ...
        public function check($plat){
            $q = $this->db->get_where('ms_mobil', array('PlatNomor' => $plat));
            if($q->num_rows() > 0){
                return true;
            }else{
                return false;
        ...
        /* API */
        public function getDetailMobil($id){
            $this->db->select('a.*, b.JenisMobil');
            $this->db->from('ms_mobil a');
            $this->db->join('ms_jenismobil b', 'a.ID_JenisMobil = b.ID_JenisMobil','inner');
        ...
        public function getDataUser($email){
            $q = $this->db->get_where('ms_pelanggan', array('Email' => $email));
            return $q->result();
        }
        
        public function getNamaUser($email){
            $q = $this->db->get_where('ms_pelanggan', array('Email' => $email));
            if($q->num_rows() > 0){
                $row = $q->row();
        ...
        public function getHistory($email){
            $this->db->select('a.*,c.PhotoURL');
            $this->db->from('tr_rental a');
            $this->db->join('ms_pelanggan b', 'a.ID_Pelanggan = b.ID_Pelanggan');
            $this->db->join('ms_mobil c', 'a.ID_Mobil = c.ID_Mobil');
            $this->db->where('b.Email',$email);
            $this->db->order_by('a.ID_Rental', 'ASC');
            $q = $this->db->get();
            return $q->result();
        }
        
        public function insertUser($data){
            $q = $this->db->insert('ms_pelanggan',$data);
            if($q) return true; else return false;
        }
        ...
        public function insertT($data){
            $q = $this->db->insert('tr_rental',$data);
            if($q) return true; else return false;
        }
    }
?>